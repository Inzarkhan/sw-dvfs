...
def create(args):
    ...
    print(
        f"DEBUG: CPU Class: {cpu_class}, Memory Mode: {mem_mode}, Want Caches: {want_caches}"
    )
    ...
    system.pci_disk_device = (
        pci_virtio_blk  # Assign as a named child attribute
    )
    ...
    if len(system._clusters) > 0:
        ...
        for cpu in cpu_cluster.cpus:
            ...
            # Removed assignment of power_model as a child SimObject to avoid cycle
            # cpu.power_model = CpuPowerModel(obj=cpu, cpu_path=cpu.path())
            print(f"Skipped direct CpuPowerModel assignment for CPU: {cpu}")
        # ...existing code...
        ...
        if hasattr(cpu_cluster, "l2"):
            ...
            if isinstance(l2_cache_candidate, SimObjectVector):
                ...
                # Removed assignment of power_model as a child SimObject to avoid cycle
                # l2_cache.power_model = L2PowerModel(obj=l2_cache, l2_path=l2_path)
                print(
                    "Skipped direct L2PowerModel assignment for L2 cache to avoid cycle."
                )
        else:
        ...
        if args.enable_dvfs:
            print(
                "DVFS is enabled. Preparing DVFSHandler for delayed assignment..."
            )
            ...
            for i, cpu in enumerate(cpu_cluster.cpus):
                ...
                print(
                    f"CPU {i} clock domain created/configured/assigned (eventq_index=0)."
                )
            ...
            print(
                "DVFSHandler instance created, configured, and eventq_index=0 set."
            )
            print(
                "DVFSHandler prepared for assignment AFTER m5.instantiate()."
            )
            system._prepared_dvfs_handler = dvfs_handler_for_later
...
def main():
    ...
    print(
        f"DEBUG: Parent of cpu_cluster.cpus[0]: {cpu_cluster.cpus[0].get_parent()}"
    )
