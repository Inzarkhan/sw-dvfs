...
class XeonCpuPowerModel(PowerModel):
    type = "XeonCpuPowerModel"
    subsystem = Param.SubSystem(
        Parent.any, "Subsystem this power model belongs to"
    )

    def computePower(self, stats):
        # Check if CPU is active by seeing if there are cycles
        if "cycles" in stats and stats["cycles"] > 0:
            # Extract IPC and L2 misses from stats
            ipc = stats["instructions"] / stats["cycles"]
            l2_misses = (
                stats["l2.overall_misses::total"] / stats["simSeconds"]
                if "simSeconds" in stats and stats["simSeconds"] > 0
                else 0
            )

            # Calculate power using the empirical formula
            power = 0.6 * ipc + 0.0000000045 * l2_misses + 3.0
            return power
        else:
            return 0  # CPU is off or not enough stats available
...
class L3PowerModel(PowerModel):
    ...
    def computePower(self, stats):
        ...
        if (
            "l3.overall_misses::total" in stats
            and "simSeconds" in stats
            and stats["simSeconds"] > 0
        ):
            l3_misses = stats["l3.overall_misses::total"] / stats["simSeconds"]
            power = 0.000000005 * l3_misses + 2.0
            return power
        else:
            return 0  # Not enough stats available
...
def build_test_system(np):
    ...
    # Set specific CPU parameters for Xeon Gold 6132
    for cpu in test_sys.cpu:
        if isinstance(cpu, DerivO3CPU):
            # Adjust parameters to match compiled limits
            cpu.numROBEntries = 192
            cpu.numPhysIntRegs = 256
            cpu.numPhysFloatRegs = 256
            cpu.numPhysVecRegs = 256
            cpu.numPhysVecPredRegs = 32
            cpu.numPhysCCRegs = 256
            # Reduce fetchWidth to match compiled limit of 12
            cpu.fetchWidth = 12
            cpu.decodeWidth = 12
            ...
            # Integrate the Xeon CPU power model
            cpu.power_model = PowerModel()

    if args.l3cache:
        test_sys.l3 = L3(clk_domain=test_sys.cpu_clk_domain)
        test_sys.l3.power_model = PowerModel()

        # Connect L3 cache to the memory bus
        test_sys.l3.mem_side = test_sys.membus.cpu_side_ports
        # Connect L2 cache to L3 if it exists
        if hasattr(test_sys, "l2"):
            test_sys.l2.mem_side = test_sys.l3.cpu_side_ports
    ...
    if args.ruby:
        ...
        if args.caches or args.l2cache or isinstance(TestCPUClass, DerivO3CPU):
...
# Add argument for boot script
parser.add_argument(
    "--boot-script",
    type=str,
    help="Path to the boot script file",
)

# Add argument to enable DVFS
parser.add_argument(
    "--enable-dvfs",
    action="store_true",
    help="Enable Dynamic Voltage and Frequency Scaling (DVFS)",
)

# Add argument to enable L3 cache
parser.add_argument(
    "--l3cache",
    action="store_true",
    help="Enable L3 cache in the simulation",
)

args = parser.parse_args()

# Set default values for Xeon Gold 6132
args.num_cpus = 28  # Number of cores
args.cpu_clock = "2.6GHz"
args.sys_clock = "2.6GHz"
...
if args.enable_dvfs:
    print("DVFS is enabled.")
    # Add any DVFS-specific initialization or configuration here
else:
    print("DVFS is disabled.")
...
# Set the boot script if provided
if args.boot_script:
    print(f"Setting boot script to: {args.boot_script}")
    test_sys.readfile = args.boot_script
